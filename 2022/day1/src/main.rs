...
fn main() {
    ...
    println!("Part One: The most amount of Calories an Elf is carrying is: {}", result);

    result += calories.iter().rev().take(2).sum::<usize>();

    println!("Part Two: The total calories of the top three Calorie-carrying Elves are: {}", result);
}
